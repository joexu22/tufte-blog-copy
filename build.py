    ...
    for figure_tag in soup.find_all('figure'):
        for child in figure_tag.children:
            if child.name == 'p':
                ...
                p1 = c[0].name == 'a' and 'footnote-ref' in c[0]['class']
                p2 = len(c) == 1
...
def convert_footnotes_to_sidenotes(soup):
    """
    Tufte CSS uses sidenotes in place of footnotes, so that the notes appear beside the text
...
def render_page(src_path, dst_path):
    print(f"rendering {src_path} -> {dst_path}")
    ...
    parts = ['pandoc', src_path, *global_pandoc_args,
        '--output=' + dst_path,
        '--template=' + template_output_path(main_template_path)]
    proc.wait()
